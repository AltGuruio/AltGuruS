...
/** News index is a work in progress — set to `false` when you are ready to ship it. */
const NEWS_PAGE_DISABLED = true;

if (NEWS_PAGE_DISABLED) {
  document.body.classList.add("news-launch-pending");

  function blockNewsNav(e) {
    ...
    if (!u.pathname.endsWith("news.html")) return;
    e.preventDefault();
  }

  document.addEventListener("click", blockNewsNav, true);
